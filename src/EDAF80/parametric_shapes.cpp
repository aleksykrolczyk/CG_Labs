                              ...
                              unsigned int const vertical_split_count)
{
	...
	glGenVertexArrays(1, /*! \todone fill me */ &data.vao);

	// To be able to store information, the Vertex Array has to be bound
	// first.
	glBindVertexArray(/*! \todone bind the previously generated Vertex Array */ data.vao);
	...
	// `data.bo`.
	glGenBuffers(1, /*! \todone fill me */ &data.bo);
	...
	glBindBuffer(GL_ARRAY_BUFFER, /*! \todone bind the previously generated Buffer */ data.bo);

	glBufferData(GL_ARRAY_BUFFER, /*! \todone how many bytes should the buffer contain? */ vertices.size() * sizeof(glm::vec3),
	...
	// from, and how it interprets it. When calling the following function,
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::vertices),
	                      /*! \todone how many components do our vertices have? */ 3,
	...
	glGenBuffers(1, /*! \todone fill me */ &data.ibo);

	// We still want a 1D-array, but this time it should be a 1D-array of elements, aka. indices!
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, /*! \todone bind the previously generated Buffer */ data.ibo);

	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /*! \todone how many bytes should the buffer contain? */ index_sets.size() * sizeof(glm::uvec3),
	             /* where is the data stored on the CPU? */index_sets.data(),
	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);

	data.indices_nb = /*! \todone how many indices do we have? */ index_sets.size() * 3 ;
...
bonobo::mesh_data parametric_shapes::createSphere(
        float const r,
        unsigned int const longitude_split_count, // up-down
        unsigned int const latitude_split_count) // left-right
{
    ...
    float const d_theta = 2*pi / (static_cast<float>(longitude_split_edges_count));
    float const d_phi = pi / (static_cast<float>(latitude_split_edges_count));

    // p(theta, phi) = (r*sin(theta)*sin(phi), -r*cos(theta), r*cos(theta)*sin(phi)),

    // normal
    // dp/dtheta     = (r*cos(theta)*sin(phi), 0,             r*cos(theta)*cos(phi)), norm=r*sin(phi)
    ...
    for (unsigned int i = 0u; i < latitude_split_vertices_count; ++i) {
        theta = .0f;
        auto sin_phi = glm::sin(phi);
        auto cos_phi = glm::cos(phi);
        ...
        for (unsigned int j = 0u; j < longitude_split_vertices_count; ++j) {
            ...
            texcoords[index] = glm::vec3(
                    static_cast<float>(j) / (static_cast<float>(longitude_split_vertices_count)),
    ...
    auto index_sets = std::vector<glm::uvec3>(2 * longitude_split_edges_count * latitude_split_edges_count);

    index = 0;
    for (unsigned int i = 0; i < latitude_split_edges_count; ++i){
        for (unsigned int j = 0; j < longitude_split_edges_count; ++j){
            index_sets[index] = glm::uvec3(
                    longitude_split_vertices_count * (i + 0u) + (j + 0u),
